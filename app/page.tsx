...
import { 
  ...
  Fingerprint, Search, Music, MapPin, Repeat, ArrowUpRight, LayoutDashboard, PieChart
} from "lucide-react";
...
const FounderSignature = ({ name, delay }: { name: string, delay: number }) => {
  return (
    <div className="relative inline-block">
      <span className="font-signature text-3xl md:text-5xl text-transparent select-none opacity-0">{name}</span>
      <motion.div
        initial={{ width: 0 }}
        whileInView={{ width: "100%" }}
        viewport={{ once: true }}
        transition={{ duration: 2.5, delay: delay, ease: "linear" }}
        className="absolute top-0 left-0 h-full overflow-hidden whitespace-nowrap"
      >
        <span className="font-signature text-3xl md:text-5xl text-white/90">{name}</span>
      </motion.div>
    </div>
  );
};
...
const Preloader = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0, transition: { duration: 0.8, ease: "easeInOut" } }} className="fixed inset-0 z-[250] bg-black flex flex-col items-center justify-center overflow-hidden px-4">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full max-w-lg">
        <motion.h1 initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} className="text-5xl md:text-9xl font-black tracking-tighter text-white uppercase italic leading-none mb-8 md:mb-12 text-center">
          DyzGO<motion.span animate={{ opacity: [1, 0.5, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="text-neonPink">.</motion.span>
        </motion.h1>
        <div className="w-64 md:w-full h-1.5 md:h-3 bg-zinc-900 rounded-full overflow-hidden relative border border-white/10 shadow-inner">
           <motion.div className="h-full bg-gradient-to-r from-neonPink via-neonPurple to-white shadow-[0_0_30px_rgba(217,70,239,0.9)]" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ type: "spring", stiffness: 50, damping: 15 }} />
        </div>
      </div>
    </motion.div>
  );
...
const EarlyAccessModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[250] flex items-center justify-center bg-black/90 backdrop-blur-md p-4" onClick={onClose}>
          <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md glass-panel rounded-[2rem] p-6 md:p-8 relative border border-neonPink/30 shadow-[0_0_50px_rgba(217,70,239,0.2)] bg-[#0a0a0a]">
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <button className="w-full py-3 md:py-4 rounded-xl bg-white text-black font-black text-base md:text-lg hover:bg-gray-200 transition-colors tracking-wide">NOTIFICARME</button>
            </form>
          </motion.div>
        </motion.div>
      )}
...
const ComparisonTable = () => {
  const features = [
    { name: "Tu Entrada", bad: "Perdida en correos", good: "Siempre En tu wallet" },
    { name: "Si no vas", bad: "Pierdes la plata", good: "Revende en la App" },
    { name: "Organízate", bad: "No sabes cuando ir", good: "Estado de fila en vivo" },
    { name: "Vida Social", bad: "Nula", good: "Conecta con tus amigos" },
    { name: "Beneficios", bad: "Ninguno", good: "Recompensas por niveles" },
  ];

  return (
    <section className="py-0 md:pt-20 md:pb-20 px-4 md:px-6">
        ...
        <div className="hidden md:block">
          <SpotlightCard className="p-0 border border-white/10 bg-black/40">
              <div className="grid grid-cols-3 border-b border-white/10 p-6 text-sm font-bold uppercase tracking-widest text-gray-500">
                <div className="text-left pl-6">Experiencia</div><div className="text-center">Otras Apps</div><div className="text-center text-neonPink">DyzGO App</div>
              </div>
              {features.map((row, i) => (
                <div key={i} className="grid grid-cols-3 border-b border-white/5 p-6 items-center hover:bg-white/5 transition-colors">
                  ...
                  <div className="text-center text-gray-500 flex flex-row justify-center items-center gap-2 text-sm font-medium"><XCircle size={14} className="text-red-900 shrink-0"/> {row.bad}</div>
                  <div className="text-center text-white font-bold flex flex-row justify-center items-center gap-2 text-sm shadow-neon"><CheckCircle2 size={16} className="text-neonPink drop-shadow-[0_0_8px_rgba(217,70,239,0.8)] shrink-0"/> {row.good}</div>
                </div>
              ))}
          </SpotlightCard>
        </div>
        <div className="md:hidden space-y-2">
          {features.map((row, i) => (
             <div key={i} className="bg-white/5 rounded-xl p-2.5 flex items-center justify-between border border-white/5">
                <div className="flex flex-col text-left"><span className="text-white font-bold text-[11px] mb-0.5">{row.name}</span><span className="text-[9px] text-gray-500 line-through decoration-red-900/50">{row.bad}</span></div>
                <div className="flex items-center gap-1.5 bg-neonPink/10 px-2 py-1 rounded-lg border border-neonPink/20"><CheckCircle2 size={10} className="text-neonPink shrink-0"/><span className="text-white font-bold text-[9px]">{row.good}</span></div>
             </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-neonPink/30 font-sans relative overflow-x-hidden">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: loading ? 0 : 1 }} transition={{ duration: 1.5 }}>
        ...
        {/* 1. HERO SECTION */}
        <section className="relative min-h-[60vh] md:min-h-[90vh] flex flex-col justify-center items-center md:items-start px-4 md:px-20 z-10 pt-16 md:pt-0">
        ...
        {/* 2. UX MODULES */}
        <section className="pt-8 pb-0 md:py-32 px-4 md:px-12 relative z-10 space-y-12 md:space-y-40">
           ...
           <div className="flex flex-col lg:flex-row gap-6 md:gap-16 items-center">
                <div className="lg:w-2/5 order-1 lg:order-1 flex flex-col items-start text-left w-full">
                   <h2 className="text-2xl md:text-7xl font-black mb-3 md:mb-8 text-white uppercase tracking-tighter leading-none">TUS DATOS, <span className="text-neonPink">UNA SOLA VEZ.</span></h2>
                   <p className="text-[11px] md:text-lg text-gray-300 leading-relaxed mb-4 md:mb-10 w-full max-w-lg">Crea tu perfil una vez y compra en un clic. Sin formularios repetidos ni pasos innecesarios cada vez que eliges un evento.</p>
                   <div className="flex flex-row gap-4 w-full max-w-sm"><FeatureTag text="Registro Único" /><FeatureTag text="Acceso inmediato" /></div>
               </div>
               <SpotlightCard className="w-full lg:w-3/5 h-[160px] md:h-[500px] border border-white/10 flex items-center justify-center p-4 md:p-12 bg-white/5 backdrop-blur-xl order-2 lg:order-2">
                    <div className="flex flex-col items-center scale-75 md:scale-100">
                        <Fingerprint className="text-neonPink w-[60px] h-[60px] md:w-[150px] md:h-[150px] mb-2 drop-shadow-neon"/>
                        <div className="text-white font-black text-lg md:text-4xl uppercase tracking-[0.2em] text-center">ID VERIFICADO</div>
                        <div className="h-1 w-20 md:w-32 bg-neonPink mt-2 shadow-[0_0_15px_#db2777]"/>
                    </div>
               </SpotlightCard>
           </div>
           ...
           <div className="w-full">
               <h2 className="text-2xl md:text-7xl font-black text-white text-center mb-6 md:mb-16 uppercase tracking-tighter leading-none">UN TOQUE <span className="text-neonPurple">BASTA.</span></h2>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                   <SpotlightCard className="p-4 md:p-12 border border-white/10 bg-white/5 backdrop-blur-xl h-[140px] md:h-[450px] flex flex-col justify-center items-start text-left">
                       <Wifi className="text-neonPurple mb-2 md:mb-8 w-[24px] h-[24px] md:w-[60px] md:h-[60px]"/>
                       <h3 className="text-lg md:text-5xl font-black text-white mb-1 md:mb-4 uppercase tracking-tighter italic leading-none">Un toque basta</h3>
                       <p className="text-[9px] md:text-lg text-gray-300 leading-relaxed max-w-sm">Conecta con nuevas personas al instante acercando tu celular. Sin búsquedas, sin códigos, solo una interacción natural y directa.</p>
                   </SpotlightCard>
                   <SpotlightCard className="p-4 md:p-12 border border-white/10 bg-white/5 backdrop-blur-xl h-[140px] md:h-[450px] flex flex-col justify-center items-start text-left">
                       <Repeat className="text-neonPink mb-2 md:mb-8 w-[24px] h-[24px] md:w-[60px] md:h-[60px]"/>
                       <h3 className="text-lg md:text-5xl font-black text-white mb-1 md:mb-4 uppercase tracking-tighter italic leading-none">Ticket Transfer</h3>
                       <p className="text-[9px] md:text-lg text-gray-300 leading-relaxed max-w-sm">Transfiere tu entrada en segundos. Sale de tu Wallet y aparece automáticamente en la de otra persona, de forma segura e inmediata.</p>
                   </SpotlightCard>
               </div>
           </div>
           ...
           <div className="w-full">
               <SpotlightCard className="w-full border border-white/10 p-0 flex flex-col lg:flex-row overflow-hidden bg-white/[0.03] backdrop-blur-2xl">
                   <div className="w-full lg:w-1/2 p-6 md:p-16 flex flex-col justify-center relative z-20 text-left items-start order-1 lg:order-1">
                     ...
                     <p className="text-[11px] md:text-lg text-gray-300 mb-4 md:mb-8 leading-relaxed w-full max-w-lg">Cada evento suma. Gana experiencia, escala en el ranking y desbloquea logros a medida que vives más experiencias.</p>
                     <div className="grid grid-cols-2 gap-3 md:gap-6 mb-4 md:mb-10 w-full max-w-sm lg:max-w-none">
                       <div className="glass-panel p-2.5 md:p-4 rounded-xl md:rounded-2xl border-neonPink/20 bg-white/5 flex flex-col items-start"><TrendingUp size={14} className="text-neonPink mb-1 md:w-5 md:h-5" /><div className="text-[9px] md:text-xs text-gray-400 font-bold uppercase">Tu Ranking</div><div className="text-sm md:text-2xl font-black text-white">#749 <span className="text-[9px] md:text-[10px] text-neonPink uppercase">GLOBAL</span></div></div>
                       <div className="glass-panel p-2.5 md:p-4 rounded-xl md:rounded-2xl border-neonPurple/20 bg-white/5 flex flex-col items-start"><Gift size={14} className="text-neonPurple mb-1 md:w-5 md:h-5" /><div className="text-[9px] md:text-xs text-gray-400 font-bold uppercase">Recompensas</div><div className="text-sm md:text-2xl font-black text-white">1 <span className="text-[9px] md:text-[10px] text-neonPurple uppercase">READY</span></div></div>
                     </div>
                     <div className="space-y-1 md:space-y-4 flex flex-col items-start"><FeatureTag text="Completa misiones" /><FeatureTag text="Sube de nivel" /><FeatureTag text="Desbloquea recompensas" /></div>
                   </div>
                   <div className="w-full lg:w-1/2 relative flex items-center justify-center p-2 md:p-10 z-20 bg-white/5 lg:bg-transparent order-2 lg:order-2 h-[160px] md:h-auto">
           ...
           <div className="flex flex-col lg:flex-row-reverse gap-6 md:gap-16 items-center">
               <div className="lg:w-1/2 order-1 lg:order-1 text-left flex flex-col items-start w-full">
                 <h2 className="text-2xl md:text-7xl font-black mb-3 md:mb-8 text-white uppercase tracking-tighter leading-none">Revende <span className="text-neonPink">seguro.</span></h2>
                 <p className="text-[11px] md:text-lg text-gray-300 leading-relaxed mb-4 md:mb-10 max-w-lg">Si no puedes ir, vende tu entrada en el marketplace oficial. Tu QR original se invalida y se genera uno nuevo para el comprador. Sin clonaciones ni riesgos.</p>
                 <div className="grid grid-cols-2 gap-2 md:gap-4 w-full max-w-sm"><FeatureTag text="Oferta tu precio" /><FeatureTag text="Reventa Segura" /><FeatureTag text="QR Único" /><FeatureTag text="Marketplace Oficial" /></div>
               </div>
               <SpotlightCard className="w-full lg:w-1/2 h-[180px] md:h-auto md:aspect-[4/3] flex items-center justify-center border border-white/10 bg-white/5 backdrop-blur-xl order-2 lg:order-2">
        ...
        {/* 5. DYZGO.+ SECTION */}
        <section id="god-mode-section" className="pt-12 pb-8 md:pt-20 md:pb-40 px-4 md:px-6 relative scroll-mt-10">
           <div className="max-w-[1800px] mx-auto">
               <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-6 md:mb-16 text-center md:text-left">
                   <div className="flex flex-col items-center md:items-start">
                     <h2 className="text-3xl md:text-9xl font-black leading-none text-white uppercase flex items-center">
                       DyzGO<span className="text-transparent bg-clip-text bg-gradient-to-r from-neonPurple to-neonPink">+</span>
                     </h2>
                   </div>
               </div>
               ...
               <SpotlightCard className="w-full rounded-[1.5rem] md:rounded-[3rem] p-3 md:p-12 relative overflow-hidden shadow-[0_0_100px_rgba(168,85,247,0.2)] border border-white/10 bg-white/5 backdrop-blur-2xl">
                   <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-12 border-b border-white/10 pb-3 md:pb-6 gap-2 md:gap-4 text-left">
                     <div className="flex flex-col"><span className="text-[9px] md:text-xs text-gray-400 font-bold tracking-widest uppercase">Control total en tus manos</span><span className="text-sm md:text-xl font-bold text-white uppercase">DYZGO EXPERIENCE</span></div>
